...
Come possiamo immaginare i vari dispotivio, dato che parialo di ambito embedded, possono essere aliemntati dalle più diverse fonti di energia quali: Wall sockets, olar panels, atteries, dynamos ognuna delle quali può fornire energia in diverse forme motivo per cui deve essere trasformata, se così vogliamo dire, per essere comaptivile dal sistema che vogliamo aliemntarre.  
...
Passiamo ora più nello specifico andiamo ad analizzare le specifiche che ci vengono richieste tra le quali troviamo: input spcififaction, outpust pecification, power dissipation and transien behavour.
...
% \subsection{Basic electrical and magnetic circuit concepts}

\subsection{Some useful definitions}

Let’s take a moment, as we did earlier, to define some terms that we’ll be using or that may have already been mentioned.
Some terms may not be directly related to the following section but will be revisited later on, so it’s still worth covering them now.
...
\subsubsection{Example of buck converter sizing}
