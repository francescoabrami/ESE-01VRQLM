...
uint32_t heaterTaskBuffer[ 1024 ];
osStaticThreadDef_t heaterTaskControlBlock;
const osThreadAttr_t heaterTask_attributes = {
  .name = "heaterTask",
  .cb_mem = &heaterTaskControlBlock,
  .cb_size = sizeof(heaterTaskControlBlock),
  .stack_mem = &heaterTaskBuffer[0],
  .stack_size = sizeof(heaterTaskBuffer),
  .priority = (osPriority_t) osPriorityNormal,
};
...
/* Definitions for controllQueue */
osMessageQueueId_t controllQueueHandle;
uint8_t controllQueueBuffer[ 1 * sizeof( uint16_t ) ];
osStaticMessageQDef_t controllQueueControlBlock;
const osMessageQueueAttr_t controllQueue_attributes = {
  .name = "controllQueue",
  .cb_mem = &controllQueueControlBlock,
  .cb_size = sizeof(controllQueueControlBlock),
  .mq_mem = &controllQueueBuffer,
  .mq_size = sizeof(controllQueueBuffer)
};
/* Definitions for MeasureEventFlag */
osEventFlagsId_t MeasureEventFlagHandle;
const osEventFlagsAttr_t MeasureEventFlag_attributes = {
  .name = "MeasureEventFlag"
};
/* Definitions for AdcDrdyEventFlag */
osEventFlagsId_t AdcDrdyEventFlagHandle;
const osEventFlagsAttr_t AdcDrdyEventFlag_attributes = {
  .name = "AdcDrdyEventFlag"
};
/* USER CODE BEGIN PV */
...
int main(void)
{
  ...
  /* creation of controllQueue */
  controllQueueHandle = osMessageQueueNew (1, sizeof(uint16_t), &controllQueue_attributes);
  ...
  /* Create the event(s) */
  /* creation of MeasureEventFlag */
  MeasureEventFlagHandle = osEventFlagsNew(&MeasureEventFlag_attributes);

  /* creation of AdcDrdyEventFlag */
  AdcDrdyEventFlagHandle = osEventFlagsNew(&AdcDrdyEventFlag_attributes);
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /**/
  LL_GPIO_ResetOutputPin(HeaterActive_GPIO_Port, HeaterActive_Pin);
  ...
  /**/
  GPIO_InitStruct.Pin = HeaterActive_Pin;
  GPIO_InitStruct.Mode = LL_GPIO_MODE_OUTPUT;
  GPIO_InitStruct.Speed = LL_GPIO_SPEED_FREQ_LOW;
  GPIO_InitStruct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
  GPIO_InitStruct.Pull = LL_GPIO_PULL_NO;
  LL_GPIO_Init(HeaterActive_GPIO_Port, &GPIO_InitStruct);
...
__weak void StartHeaterTask(void *argument)
{
